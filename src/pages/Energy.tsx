...
export default function Energy() {
  ...
  const chartData = recentReadings.slice(-20).map((reading) => {
    ...
    if (reading.created_at?.toDate) {
      // Firestore Timestamp
      date = reading.created_at.toDate();
    } else {
    ...
    return {
      time: isNaN(date.getTime()) ? "-" : format(date, "HH:mm"),
  ...
  const powerQuality =
    currentEnergyData.powerFactor > 0.9
      ? "Excellent"
      : currentEnergyData.powerFactor > 0.8
        ? "Good"
        : currentEnergyData.powerFactor > 0.7
          ? "Fair"
          : "Poor";
    ...
    <div className="flex-1 space-y-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {currentEnergyData.voltage.toFixed(1)} V
            </div>
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
              <span>
                Frequency: {currentEnergyData.frequency.toFixed(1)} Hz
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {currentEnergyData.current.toFixed(3)} A
            </div>
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
              <span>
                Power Factor: {currentEnergyData.powerFactor.toFixed(2)}
              </span>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {currentEnergyData.power.toFixed(1)} W
            </div>
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {currentEnergyData.energy.toFixed(3)} kWh
            </div>
            <div className="flex items-center gap-1 text-xs text-muted-foreground">
          ...
          <div className="grid gap-4 md:grid-cols-3">
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">
                  {currentEnergyData.powerFactor.toFixed(2)}
                </span>
              </div>
              <Progress
                value={currentEnergyData.powerFactor * 100}
                className="h-2"
              />
              <Badge
                variant={
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                ...
                <span className="font-medium">
                  {Math.abs(currentEnergyData.voltage - 220) < 10
                    ? "Stable"
                    : "Unstable"}
                </span>
              </div>
            ...
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span>Frequency</span>
                <span className="font-medium">
                  {Math.abs(currentEnergyData.frequency - 50) < 1
                    ? "Normal"
                    : "Deviation"}
                </span>
              </div>
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  Real-time power usage (last 20 readings)
                </CardDescription>
              </CardHeader>
              <CardContent>
                ...
                <div className="grid gap-4 grid-cols-2">
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {totalDailyEnergy.toFixed(3)} kWh
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold">
                      {peakPower.toFixed(1)} W
                    </p>
                  </div>
                  <div>
                    <p className="text-sm text-muted-foreground">Est. Cost</p>
                    <p className="text-2xl font-bold">
                      Rp {(totalDailyEnergy * 1500).toLocaleString()}
                    </p>
                  </div>
                </div>
              </CardContent>
                    ...
                    {dailyConsumption.map((device, index) => (
                      <div key={index} className="space-y-2">
                        <div className="flex justify-between text-sm">
                          ...
                          <span className="font-medium">
                            {Number(device.total_energy).toFixed(3)} kWh
                          </span>
                        </div>
                        <Progress
                          value={
                            totalDailyEnergy > 0
                              ? (Number(device.total_energy) /
                                  totalDailyEnergy) *
                                100
                              : 0
                          }
                        ...
                        <div className="flex justify-between text-xs text-muted-foreground">
                          <span>
                            Avg: {Number(device.avg_power).toFixed(1)}W
                          </span>
                          <span>
                            Peak: {Number(device.peak_power).toFixed(1)}W
                          </span>
                          <span>
                            Runtime: {Number(device.runtime_hours).toFixed(1)}h
                          </span>
                        </div>
                      </div>
                    ))}
        ...
        <TabsContent value="analysis" className="space-y-4">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                System efficiency and optimization suggestions
              </CardDescription>
            </CardHeader>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="flex justify-between">
                      ...
                      <Badge
                        variant={
                          powerQuality === "Excellent" ? "default" : "secondary"
                        }
                      >
                        {powerQuality}
                      </Badge>
                    ...
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium">
                        Rp {(totalDailyEnergy * 1500).toLocaleString()}/day
                      </span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="p-3 bg-muted rounded-lg">
                      ...
                      <p>
                        Monitor power factor to maintain above 0.85 for optimal
                        efficiency
                      </p>
                    </div>
                    <div className="p-3 bg-muted rounded-lg">
                    ...
                    <div className="p-3 bg-muted rounded-lg">
                      ...
                      <p>
                        Regular sensor calibration ensures accurate readings
                      </p>
                    </div>
                  </div>
                </div>
