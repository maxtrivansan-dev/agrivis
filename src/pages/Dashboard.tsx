...
export default function Dashboard() {
  ...
  const { updateDeviceControl, loading } = useFirebaseSettings();
  const [selectedValve, setSelectedValve] = useState<"water" | "vitamin">(
    "water",
  );

  const handleValveSelection = async (valveType: "water" | "vitamin") => {
    const waterValve = getDeviceByName("water_valve");
    const vitaminValve = getDeviceByName("vitamin_valve");

    if (!waterValve || !vitaminValve) return;

    try {
      if (valveType === "water") {
        await updateDeviceControl("water_valve", { is_active: true });
        await updateDeviceControl("vitamin_valve", { is_active: false });
      } else {
        await updateDeviceControl("water_valve", { is_active: false });
        await updateDeviceControl("vitamin_valve", { is_active: true });
      }
      setSelectedValve(valveType);
      toast.success(
        `${valveType === "water" ? "Water" : "Vitamin"} valve selected`,
      );
    } catch (error) {
      console.error("Error selecting valve:", error);
      toast.error("Failed to select valve");
    }
  };
  ...
  const waterValveDevice = getDeviceByName("water_valve");
  const vitaminValveDevice = getDeviceByName("vitamin_valve");
  ...
  const getTemperatureStatus = () => {
    if (!sensorData?.temperature || !thresholds?.max_temperature)
      return "normal";
    return sensorData.temperature > thresholds.max_temperature
      ? "warning"
      : "normal";
  };

  const getSoilMoistureStatus = () => {
    if (!sensorData?.soil_moisture || !thresholds?.min_soil_moisture)
      return "normal";
    return sensorData.soil_moisture < thresholds.min_soil_moisture
      ? "warning"
      : "normal";
  };

  const getLightLevelStatus = () => {
    if (!sensorData?.light_level || !thresholds?.min_light_level)
      return "normal";
    return sensorData.light_level < thresholds.min_light_level
      ? "warning"
      : "normal";
  };

  const getFlowRateStatus = () => {
    if (!sensorData?.flow_rate || !thresholds?.min_flow_rate) return "normal";
    return sensorData.flow_rate < thresholds.min_flow_rate
      ? "warning"
      : "normal";
  };

  const getTotalFlowVolumeStatus = () => {
    if (!sensorData?.total_flow_volume || !thresholds?.min_total_flow_volume)
      return "normal";
    return sensorData.total_flow_volume < thresholds.min_total_flow_volume
      ? "warning"
      : "normal";
  };
  ...
  return (
    <div className="flex-1 space-y-6 p-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        ...
        <SensorCard
          title="Flow Rate"
          value={sensorData?.flow_rate ?? null}
          unit=" L/min"
          icon={Wind}
          status={getFlowRateStatus()}
          description="Water flow rate"
        />

        <SensorCard
          title="Total Flow Volume"
          value={sensorData?.total_flow_volume ?? null}
          unit=" L"
          icon={Droplets}
          status={getTotalFlowVolumeStatus()}
          description="Total water dispenser"
        />
            ...
            <CardContent className="space-y-6">
              ...
              <div className="grid gap-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={pumpDevice?.is_active ? "default" : "secondary"}
                    >
                      {pumpDevice?.is_active ? "ON" : "OFF"}
                    </Badge>
                    <Button
                      variant={
                        pumpDevice?.is_active ? "destructive" : "default"
                      }
                      size="sm"
                      disabled={autoMode || loading}
                      onClick={() => handleDeviceToggle("pump")}
                    >
                      {pumpDevice?.is_active ? (
                ...
                {/* Valve Selection - hanya tampil jika pump aktif atau auto mode off */}
                {(!autoMode || pumpDevice?.is_active) && (
                  <div className="space-y-3 p-4 bg-muted/50 rounded-lg">
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <Button
                        variant={
                          waterValveDevice?.is_active ? "default" : "outline"
                        }
                        size="sm"
                        disabled={autoMode || loading}
                        onClick={() => handleValveSelection("water")}
                        className="w-full"
                      >
                      ...
                      <Button
                        variant={
                          vitaminValveDevice?.is_active ? "default" : "outline"
                        }
                        size="sm"
                        disabled={autoMode || loading}
                        onClick={() => handleValveSelection("vitamin")}
                        className="w-full"
                      >
                    ...
                    <p className="text-xs text-muted-foreground">
                      Select which valve to use with the pump
                    </p>
                  </div>
                )}
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    <Badge
                      variant={fanDevice?.is_active ? "default" : "secondary"}
                    >
              ...
              {autoMode && (
                <div className="text-sm text-muted-foreground bg-muted p-3 rounded-lg">
                  <div className="flex items-center gap-2">
                    ...
                    <span>
                      Auto mode enabled - devices controlled by schedule and
                      thresholds
                    </span>
                  </div>
                </div>
